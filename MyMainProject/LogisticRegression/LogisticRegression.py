...
# Вывод: Метод работает достаточно точно (~95.3%). 
# Его результат в моём случае не зависел от дополнительного подбора гиперпараметров при оценке с точностью до 10^(-16)  (16 знаков после запятой)
# Показал достаточно высокую точность на обучающем наборе данных (~96.5%)

# TODO Обработать выбросы и применить метод повторно.