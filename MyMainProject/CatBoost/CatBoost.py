...
def CatBoost(features, Class):
    ...
    #rid_search.fit(X_train, y_train)
...
# Вывод: Новая модель классификации от Yandex показала отличные результаты. 
# Точность на обучающем наборе совпала с точностью на обучающем наборе при использовании других бустингов.
# Модель использует автоматическую настройку гипперпараметров, результат которой совпал с ручной настройкой.
# Учитывая, что этот метод предназначен для большого объёма данных, он показал отличные результаты на текущем небольшом датасете. 